...
%      corner of the current axes and returns its handle.
%
%   h = axes_label('label', dx, dy)
...
function h = axes_label(varargin)
...
function posChanged(~, ~, h, dx, dy)
    ...
    set(h, 'Units', 'pixels');
    p = get(h, 'Position');
    set(h, 'Position', [p(1)+5+dx, p(2)+10-dy]);
